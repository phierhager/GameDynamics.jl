...
"""
Exact LP solver for a 2-player zero-sum normal-form game.

Returns:
    σ1, σ2, value
where σ1 and σ2 are `FiniteMixedStrategy`s.
"""
function solve_zero_sum_nash(game::NormalForm.NormalFormGame{2};
    ...
    σ1 = Strategies.FiniteMixedStrategy(Base.OneTo(m), xval)
    σ2 = Strategies.FiniteMixedStrategy(Base.OneTo(n), yval)
...
"""
Max-social-welfare correlated equilibrium for a finite normal-form game.

Returns a `CorrelatedStrategy`.
"""
function solve_ce(game::NormalForm.NormalFormGame{N};
    ...
    return Strategies.CorrelatedStrategy(prof_support, prob_support)
end

"""
Max-social-welfare coarse correlated equilibrium for a finite normal-form game.
