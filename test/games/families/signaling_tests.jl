...
@testset "Signaling" begin
    ...
    @testset "SignalingProfile accessors and sampling" begin
        sender_strat = DirectDecisionRules.FiniteMixedDecisionRule((:m1, :m2), (1.0, 0.0))
        receiver_strat = DirectDecisionRules.FiniteMixedDecisionRule((:a1, :a2), (0.0, 1.0))
    ...
    struct SenderBehavior <: DecisionRulesInterface.AbstractDecisionRule end
    DecisionRulesInterface.local_rule(::SenderBehavior, ::Symbol) = DirectDecisionRules.FiniteMixedDecisionRule((:m1, :m2), (0.75, 0.25))
    DecisionRulesInterface.local_rule(::SenderBehavior, ::Int) = DirectDecisionRules.FiniteMixedDecisionRule((:m2, :m3), (0.2, 0.8))
    ...
    @testset "induced_message_distribution over finite sender type space" begin
        ...
        @test d isa DirectDecisionRules.FiniteMixedDecisionRule
        supp = DecisionRulesInterface.support(d)
        probs = DecisionRulesInterface.probabilities(d)

        @test supp == (:m1, :m2, :m3)
        @test isapprox(sum(probs), 1.0)
        @test isapprox(DecisionRulesInterface.action_probability(d, :m1), 0.4 * 0.75)
        @test isapprox(DecisionRulesInterface.action_probability(d, :m2), 0.4 * 0.25 + 0.6 * 0.2)
        @test isapprox(DecisionRulesInterface.action_probability(d, :m3), 0.6 * 0.8)
    end

    @testset "induced_message_distribution rejects empty local support" begin
        struct BadSenderBehavior <: DecisionRulesInterface.AbstractDecisionRule end
        DecisionRulesInterface.local_rule(::BadSenderBehavior, t) = DirectDecisionRules.FiniteMixedDecisionRule((), ())
