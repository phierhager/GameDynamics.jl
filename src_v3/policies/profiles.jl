...
@inline function _sample_profile_action_impl(policies::Tuple,
                                             game::Kernel.AbstractGame,
                                             state,
                                             rng::AbstractRNG,
                                             query_action)
    _validate_profile_length(policies, game)
...
function sample_profile_action(profile::UnconditionedPolicyProfile,
    ...
    return _sample_profile_action_impl(profile.policies, game, state, rng) do p, st, rr
        PolicyCore.sample_action(profile.policies[p], rr)
    end
end
...
function sample_profile_action(profile::ObservationPolicyProfile,
    ...
    return _sample_profile_action_impl(profile.policies, game, state, rng) do p, st, rr
        ...
        PolicyCore.sample_action(profile.policies[p], obs, rr)
    end
end
...
function sample_profile_action(profile::StatePolicyProfile,
                               ...
                               rng::AbstractRNG = Random.default_rng())
    return _sample_profile_action_impl(profile.policies, game, state, rng) do p, st, rr
        PolicyCore.sample_action(profile.policies[p], st, rr)
    end
end
