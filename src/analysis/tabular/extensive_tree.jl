module TabularExtensiveTrees

using ..TabularTraits

export TabularExtensiveTree
...
struct TabularExtensiveTree{IE,AL} <: TabularTraits.AbstractTabularExtensiveFormModel
    n_players::Int
...
n_nodes(g::TabularExtensiveTree) = g.n_nodes
n_infosets(g::TabularExtensiveTree) = g.n_infosets
node_action_count(g::TabularExtensiveTree, node::Int) = g.node_len[node]
infoset_player(g::TabularExtensiveTree, infoset::Int) = g.infoset_player[infoset]
node_infoset(g::TabularExtensiveTree, node::Int) = g.node_infoset[node]
node_player(g::TabularExtensiveTree, node::Int) = g.node_player[node]
has_simultaneous_nodes(g::TabularExtensiveTree) = g.has_simultaneous

@inline function action_range(g::TabularExtensiveTree, node::Int)
    a0 = g.node_first[node]
    return a0:(a0 + g.node_len[node] - 1)
end

@inline function infoset_range(g::TabularExtensiveTree, infoset::Int)
    i0 = g.infoset_offset[infoset]
    return i0:(g.infoset_offset[infoset + 1] - 1)
end

chance_probabilities(g::TabularExtensiveTree, node::Int) =
    @view g.chance_prob[action_range(g, node)]

function active_players(g::TabularExtensiveTree, node::Int)
...
infoset_action_labels(g::TabularExtensiveTree, infoset::Int) =
    g.infoset_action_label[infoset_range(g, infoset)]

TabularTraits.is_tree_model(::TabularExtensiveTree) = true
TabularTraits.supports_exact_solvers(::TabularExtensiveTree) = true
TabularTraits.supports_approx_solvers(g::TabularExtensiveTree) = !g.has_simultaneous
